...
//I used an adjacency Matrix for implementing the graph, because the algorithm seems easier to understand in this way for me
...
public:
	...
	//this function gives a matrix that encodes the function generated in Floyds algorithm, so the numbers represent the weighted pathlength and 0 means that there is no path
	int** Floyd() {
		int** C = new int* [dim];
		for (int i = 0; i < dim; i++) {
			C[i] = new int[dim]();
